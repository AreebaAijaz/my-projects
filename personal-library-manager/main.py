...
class BookCollection:
    ...
    def save_to_file(self):
        """Save the current book list to a JSON file."""
        with open(self.storage_file, 'w') as file:
            json.dump(self.book_list, file, indent=4)

    def create_new_book(self):
        """Add a new book to the collection by gathering user input."""
        ...
        self.save_to_file()
        print(f"Book {book_title} successfully added to the collection.")
    ...
    def find_book(self):
        """Search for a book by the title or the name of the author."""
        ...
        if found_books:
            ...
            for index, book in enumerate(found_books, 1):
                ...
                print(f"{index}. {book['title']} by {book['author']} ({book['year']}) - Genre: {book['genre']} - Read: {'Yes' if book['read'] else 'No'}")
        else:
